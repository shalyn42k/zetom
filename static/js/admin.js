...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const quoteModal = $('[data-quote-modal]');
        if (!quoteModal) {
            return;
        }

        const replyButtons = $$('[data-reply-button]');
        const form = $('[data-quote-form]', quoteModal);
        const confirmButton = $('[data-quote-confirm]', quoteModal);
        const closeElements = $$('[data-quote-close]', quoteModal);
        const backdrop = quoteModal.querySelector('.modal__backdrop');
        const emailForm = document.querySelector('[data-email-form]');
        const formsPanelTrigger = document.querySelector('[data-panel-trigger][data-panel="forms"]');
        ...
        const getRowData = (row) => {
            ...
            const dateTime = row.dataset.date || '';
            ...
            return {
                ...
                date: datePart || '',
                time: timePart || '',
                message: normalizeMessage(row.dataset.message || ''),
                company: row.dataset.company || '',
            };
        };

        const buildQuote = (data, options) => {
            const lines = [];
            const dateLabel = options.datetime && data.date
                ? data.time
                    ? `W dniu ${data.date} o ${data.time}`
                    : `W dniu ${data.date}`
                : '';
            const senderLabel = options.sender && data.fullName ? `${data.fullName} pisze` : '';
            let header = '';
            if (dateLabel && senderLabel) {
                header = `${dateLabel}, ${senderLabel}:`;
            } else if (dateLabel) {
                header = `${dateLabel}:`;
            } else if (senderLabel) {
                header = `${senderLabel}:`;
            ...
            if (options.message && data.message) {
        ...
        const applyQuoteToForm = (row) => {
            if (!emailForm || !form || !row) {
                return;
            }
            const data = getRowData(row);
            const quoteTextField = emailForm.querySelector('[name="quote_text"]');
            const messageField = emailForm.querySelector('[name="message_text"]');
            const toEmailField = emailForm.querySelector('[name="to_email"]');
            const subjectField = emailForm.querySelector('[name="subject"]');
            const options = {
                datetime: form.elements.namedItem('quote_datetime')?.checked,
                sender: form.elements.namedItem('quote_sender')?.checked,
                contact: form.elements.namedItem('quote_contact')?.checked,
                message: form.elements.namedItem('quote_message')?.checked,
            };
            const quote = buildQuote(data, options);

            if (toEmailField) {
                toEmailField.value = data.email || '';
            }
            if (subjectField) {
                const id = data.id ? `#${data.id}` : '';
                subjectField.value = id ? `Odp: Zgłoszenie ${id}` : 'Odp: Zgłoszenie';
            }
            if (quoteTextField) {
                quoteTextField.value = quote;
            }
            if (messageField) {
                messageField.value = '';
                messageField.focus();
        ...
        if (confirmButton) {
            confirmButton.addEventListener('click', () => {
                ...
                applyQuoteToForm(currentRow);
                if (formsPanelTrigger) {
                    formsPanelTrigger.click();
                }
                toggleModal(false);
            });
        }
        ...
        document.addEventListener('keydown', (event) => {
            if (event.key === 'Escape' && quoteModal.classList.contains('is-visible')) {
                toggleModal(false);
            }
        });
    });
