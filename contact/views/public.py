...
def index(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == 'POST':
        ...
        if remaining_durations:
            ...
            form.add_error(
                None,
                _("Please wait %(seconds)s s before submitting the form again.")
    ...
    if request.method == 'POST' and form_valid and not throttle_error:
        ...
        try:
            ...
            form.add_error(
                None,
                _('Unable to save the request right now. Please try again later.'),
            )
        else:
            try:
                ...
                form.add_error(
                    None,
                    _('Unable to send the email right now. Please try again later.'),
                )
            else:
